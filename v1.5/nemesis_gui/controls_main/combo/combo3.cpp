...
namespace ui {
	...
	void combo::draw( ) {
		...
		auto combo_drawing = [ ]( float x, float y, float w, float h, std::string name, std::string selected, bool focuseed) -> void {
		...
		combo_drawing( pos.x + 5, pos.y + 14, 350, 23, name, items[ *value ], focused );
		...
		if ( animation[ ui::m_control.get_index( ) ] != 0 ) {
			for ( size_t i{ 0 }; i < items.size( ); i++ ) {
				...
				color_t m_Select = ui::m_input.mouse_pointer( { pos.x, pos.y + 41 + ( i * 23 ) }, { 350, 23 } ) || *value == i ? settings.modify_menu_color ? settings.accent_color : accent : color_t( 120, 120, 120, 150 );
				ui::m_ext_render.text_draw.insert( ui::m_ext_render.text_draw.begin( ), ui::external::e_draw_text( pos.x + 10, pos.y + 45 + ( 23 * i ), items[ i ], m_Select ) );

				ui::m_ext_render.rect_draw.insert( ui::m_ext_render.rect_draw.begin( ), ui::external::e_draw_rect( pos.x + 6, pos.y + 41 + ( 23 * i ), 350 - 2, 22, *value == i ? color_t( 40, 40, 40 ) : color_t( 35, 35, 35, 0 ) ) );
			}
